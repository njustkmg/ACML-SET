...
class ResNetVLBERTForPretrainingMultitask(CapModule):
    ...
    def forward(self,
        ...
        relationship_logits_multi, mlm_logits_multi, mvrc_logits_multi, con_loss, sty_loss = self.vlbert(text_input_ids_multi,
        ...
        outputs.update({
            ...
            'mvrc_loss': mvrc_loss,
            'contractive_loss': con_loss,
            'style_loss': sty_loss,
        })

        loss = (relationship_loss.mean() + mlm_loss_wvc.mean() + mlm_loss_aux.mean() + mvrc_loss.mean()) + con_loss.mean() + style_loss.mean()
