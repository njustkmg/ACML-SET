...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence,  vl_out: utils.TensorOrSequence, 
                    extended_attention_mask: utils.TensorOrSequence, out_size=1, 
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual, outputs = self.iter(t, visual, vl_out, extended_attention_mask, outputs, return_probs, **kwargs)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, vl_out: utils.TensorOrSequence, 
                           extended_attention_mask: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, vl_out, extended_attention_mask, self.selected_words, visual, None, mode='feedback', **kwargs)
