...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        if 'tag_text' in self.data_names:
            max_tag_length = max([len(data[self.data_names.index('tag_text')]) for data in batch])
        ...
        for i, ibatch in enumerate(batch):
            ...
            if 'tag_text' in self.data_names:
                tag_text = ibatch[self.data_names.index('tag_text')]
                out['tag_text'] = clip_pad_1d(tag_text, max_tag_length, pad=0)
