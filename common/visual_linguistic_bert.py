...
# import torch
from common.utils.loss import contractive_loss, style_loss
...
class VisualLinguisticBert(BaseModel):
    def __init__(self, config, language_pretrained_model_path=None):
        ...
        if self.config.with_pooler:
            self.pooler = BertPooler(config) # 一个fc
    ...
    def forward(self,
        ...
        if output_text_and_object_separately:
            ...
            # 这样分离之后返回的就是一个list，而不是一个tensor了，如何保证梯度的不变？
            ...
            if output_attention_probs:
                return encoded_layers, encoded_layers_text, encoded_layers_object, pooled_output, attention_probs, extended_attention_mask
            else:
                return encoded_layers, encoded_layers_text, encoded_layers_object, pooled_output, extended_attention_mask
        else:
            if output_attention_probs:
                return encoded_layers, pooled_output, attention_probs, extended_attention_mask, embedding_output
            else:
...
class VisualLinguisticBertForPretraining(VisualLinguisticBert):
    ...
    def forward(self,
        ...
        vl_out, text_out, object_out, pooled_rep, extended_attention_mask = super(VisualLinguisticBertForPretraining, self).forward(
            text_input_ids,
            text_token_type_ids,
            text_visual_embeddings,
            text_mask,
...
class VisualLinguisticBertForCaptioning(VisualLinguisticBert):
    def __init__(self, config, language_pretrained_model_path=None,
                 with_rel_head=True, with_mlm_head=True, with_mvrc_head=True):

        super(VisualLinguisticBertForCaptioning, self).__init__(config, language_pretrained_model_path=None)
    ...
    def forward(self,
        ...
        encoded_layers, encoded_layers_text, encoded_layers_object, pooled_output, extended_attention_mask = super(VisualLinguisticBertForCaptioning, self).forward(
            text_input_ids,
            text_token_type_ids,
            text_visual_embeddings,
            text_mask,
        ...
        # print("text out: ", text_out.size())
        # print("visual out: ", object_out.size())
        # text out:  torch.Size([34, 64, 768])
        # visual out:  torch.Size([34, 14, 768])
        # text_out, object_out分别为文本是视觉区域特征，可将其进行对应位置拼接
        # extended_attention_mask为padding的mask矩阵，但是将二者对应拼接之后这个mask矩阵也应该跟着变化
...
class VisualLinguisticBertMVRCHead(BaseModel):
    def __init__(self, config):
        ...
        # self.region_cls_pred_fitment = nn.Linear(config.visual_region_classes, 121)
        self.apply(self.init_weights)
    ...
    def forward(self, hidden_states):
        ...
        # logits = self.region_cls_pred_fitment(logits)
